...
public class ProductServiceImpl implements ProductService {
        ...
        try {
            ...
            allProducts = allProducts.stream()
                .filter(product -> {
                    ...
                    // 分类筛选
                    if (queryDto.getCategoryId() != null && !queryDto.getCategoryId().equals(product.getCategoryId())) {
                        return false;
                    }
    ...
    public boolean updateProductPrice(Long productId, Double newPrice, String reason, Long operatorId) {
        ...
        try {
            ...
            // 记录价格变更历史
            recordPriceHistory(productId, null, oldPrice, newPrice, "PRODUCT", reason, operatorId, null, product.getName(), null);
        ...
        try {
            ...
            for (ProductService.PriceUpdate update : priceUpdates) {
                ...
                if (update.getProductId() != null) {
                    success = updateProductPrice(update.getProductId(), update.getNewPrice(), update.getReason(), update.getOperatorId());
                } else if (update.getSkuId() != null) {
                    success = updateSkuPrice(update.getSkuId(), update.getNewPrice(), update.getReason(), update.getOperatorId());
                }
