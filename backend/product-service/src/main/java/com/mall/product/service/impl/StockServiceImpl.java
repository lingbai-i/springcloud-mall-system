...
public class StockServiceImpl implements StockService {
    ...
    public StockOperationResult deductStock(Long productId, Long skuId, Integer quantity, String orderNo, Long operatorId) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
    public StockOperationResult rollbackStock(Long productId, Long skuId, Integer quantity, String orderNo, Long operatorId) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public StockOperationResult stockTaking(Long productId, Long skuId, Integer actualStock, Long operatorId, String reason) {
     ...
     * @since 2025-01-21
     */
    private StockOperationResult deductSkuStock(Long productId, Long skuId, Integer quantity, String orderNo, Long operatorId) {
        // 增强参数验证
        if (productId == null) {
     ...
     * @since 2025-01-21
     */
    private StockOperationResult rollbackProductStock(Long productId, Integer quantity, String orderNo, Long operatorId) {
        // 增强参数验证
        if (productId == null) {
     ...
     * @since 2025-01-21
     */
    private StockOperationResult rollbackSkuStock(Long productId, Long skuId, Integer quantity, String orderNo, Long operatorId) {
        // 增强参数验证
        if (productId == null) {
    ...
    private StockOperationResult stockTakingProduct(Long productId, Integer actualStock, Long operatorId, String reason) {
        Product product = PRODUCT_CACHE.get(productId);
        if (product == null) {
            return new StockOperationResult(false, "商品不存在");
        }
        ...
        // 记录库存变更日志
        Long logId = recordStockLog(productId, null, oldStock, actualStock, changeQuantity, "4", reason != null ? reason : "库存盘点", null, "管理员", product.getName(), null);
        
        return new StockOperationResult(true, "盘点成功", oldStock, actualStock, logId);
    }
    
    private StockOperationResult stockTakingSku(Long productId, Long skuId, Integer actualStock, Long operatorId, String reason) {
        List<ProductSku> skuList = SKU_CACHE.get(productId);
        if (skuList == null) {
            return new StockOperationResult(false, "商品SKU不存在");
        }
        ...
        // 记录库存变更日志
        Long logId = recordStockLog(productId, skuId, oldStock, actualStock, changeQuantity, "4", reason != null ? reason : "库存盘点", null, "管理员", sku.getProductName(), sku.getSkuName());
    ...
    private String getStockActionByChangeType(String changeType) {
        if (changeType == null) return "STOCK_UNKNOWN";
        
        switch (changeType) {
            case "1": return "STOCK_INCREASE";
            case "2": return "STOCK_DECREASE";
            case "3": return "STOCK_TRANSFER";
            case "4": return "STOCK_INVENTORY";
            default: return "STOCK_OTHER";
        }
    }
    ...
    private static void initMockData() {
        ...
        logger.info("库存服务模拟数据初始化完成 - 商品数量: {}, SKU总数: {}", 
            PRODUCT_CACHE.size(), SKU_CACHE.values().stream().mapToInt(List::size).sum());
    }
            ...
            try {
                ...
                if (rollbackResult.isSuccess()) {
                    ...
                    logger.error("回滚第{}个操作失败 - 批次ID: {}, 商品ID: {}, SKU ID: {}, 原因: {}", 
                        i + 1, batchId, operation.getProductId(), operation.getSkuId(), rollbackResult.getMessage());
                }
            } catch (Exception e) {
                rollbackFailCount++;
